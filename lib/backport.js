...
const backportBanis = async () => {
  ...
  let banis = await readJson( banisPath )

  // Remove duplicates
  banis = banis.filter( ( value, index, self ) => index === self.findIndex( test => (
    test.name_gurmukhi === value.name_gurmukhi && test.name_english === value.name_english
  ) ) )
