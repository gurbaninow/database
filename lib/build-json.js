...
const processBanis = async () => {
  ...
  return knex( 'banis' ).select().orderBy( 'order_id' )
    .then( banis => banis.reduce( async ( compiled, {
      ...
      // Resolve each group range by order_id, then map back to line ids.
      const lines = await knex
        .with( 'ranges', qb => (
          qb
            .from( { bl: 'bani_lines' } )
            .join( { l: 'lines' }, 'l.id', 'bl.line_id' )
            .where( 'bl.bani_id', id )
            .groupBy( 'bl.line_group' )
            .select( 'bl.line_group' )
            .min( 'l.order_id as min_order_id' )
            .max( 'l.order_id as max_order_id' )
        ) )
        .from( { r: 'ranges' } )
        .join( { lmin: 'lines' }, 'lmin.order_id', 'r.min_order_id' )
        .join( { lmax: 'lines' }, 'lmax.order_id', 'r.max_order_id' )
        .select( [ 'lmin.id as start_line', 'lmax.id as end_line' ] )
        .orderBy( 'r.line_group' )
